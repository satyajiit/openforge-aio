...
**Property-name hash — VALIDATED.** Property/pin names hash with **standard CRC32** (IEEE/zlib, poly `0xEDB88320`, reflected). Computed CRC32 of 6 names matched the live `m_nPropertyID`s exactly (`m_infiniteAmmo`=`0x2BD1CEC3`, `m_humanoid`=`0xA18C1668`, `m_maxhealth`=`0xA92A6D3C`, …). Locked in a `crates/glacier-host` unit test. (Type *names* use FNV-1a-64-lower in the registry; only properties/pins use CRC32.)

**Live per-instance offsets — IMPLEMENTED per ZHMModSDK; live end-to-end gated on instance discovery.** `glacier-host::resolve_instance_property` walks `ZEntityImpl.m_pEntityType` @ `+0x08` (**tagged ptr** — bit0 set ⇒ self-relative indirection `*(&slot + (raw>>1))`) → `ZEntityType.m_pPropertyData` @ `+0x08` (a *pointer to* `TArray<SPropertyData>{begin@0,end@8}`) → linear scan `SPropertyData` (stride `0x28`): `m_pPropertyInfo @+0x00`, `m_nPropertyOffset` (i64) `@+0x08`, `CRC32 id @+0x10`, flags `@+0x14`; the getter/setter flag lives on the descriptor (`m_pPropertyInfo` → `SPropertyInfo.m_Flags @+0x10`, name via `m_pPropertyInfo - 0x10`). The forward walk is correct per the SDK source; it is **not yet live-validated end-to-end** because we cannot cheaply obtain a live entity instance (see below).
...
### 3.4 Validation oracle — RESOLVED: the ammo is NOT a clean entity field
- The ammo magazine value `0x18F272A90` (=38) is a **pooled value-box, not a plain reflected entity property**. Its struct base reads all-zeros; a pool record `{ptr, cap=8}` at `0x19015DBE8` owns it, and nothing points into it within 512 bytes. Runtime gameplay state (ammo/health) lives in value-boxes / `PlanScoped` providers — reflected entity properties are mostly **spawn-time config** (`ZFirearmCharacterEntity` reflects only `m_maximum/minimumAmmunitionWhenSpawned`, *not* the live clip). So the oracle is **not** a usable per-instance-offset gate.
- **Entity instances do NOT back-reference the reflection `IType`.** The only heap refs to a class IType are open-addressing hashmap nodes (`0x18`-stride `{CRC32, STypeID*, val|0xFFFFFFFF}`), not per-instance `ZEntityType`s. ⇒ "scan for IType backrefs" does **not** find live instances; there is no cheap external instance-discovery. A live entity must be reached via a stable anchor (player/scene-context global) or **in-process enumeration (Tier-2 DLL)**.
...
- **Phase 1 — Reflection probe (research → tooling). DONE.** `glacier-host` + `glacier-walk` (type+property enum) + `glacier-entity` (live instance walk). Type + static-property layers validated live; CRC32 property hash validated + unit-tested.
- **Phase 2 — Property resolution + findings. DONE (with a pivot).** Instance→property forward walk implemented per ZHMModSDK (`resolve_instance_property`). Key findings: (a) the ammo "oracle" is a pooled value-box, not a reflected field, so it can't gate per-instance offsets; (b) instances don't back-reference the IType, so there is **no cheap external instance discovery**; (c) ZCL gameplay nodes need on-thread actuation (SignalInputPin/SetPropertyValue). ⇒ gameplay is **Tier-2**. The forward walk's live end-to-end validation is deferred to when a live entity anchor exists (Tier-2 enumeration, or a player-global signature).
- **`glacier-007` registered. DONE.** Promoted to a real game crate (manifest + build.rs + `register_game!` + bundle + workspace); **lists + is selectable in the app** with zero frontend changes. Zero features yet; `dll_file_name=""`.
- **Phase 3 — Backend + first feature (BLOCKED on a decision).** Two paths: **(A) Tier-1 RPM** — find a stable player/scene anchor, add `[glacier_reflection]` features over a `Target`-as-`Ctx` dispatch (`dll_file_name==""` branch in attach.rs/commands.rs); or **(B) Tier-2 DLL** — go straight to the injected `glacier-dll` (below), the user-endorsed hook approach, which the gameplay features require anyway. Building (A)'s RPM dispatch is likely thrown away if we commit to (B). **Decide before the big build.**
...
- **ZCL actuation needs Tier-2 (RESEARCHED).** Cripple entities are absent; First Light exposes native nodes (`ZCLSetHumanoidImmuneToDamage`, `ZCLSetFirearmInfiniteAmmo`, …). Their inputs are value-provider refs (`m_invulnerable: TInterfaceRef<IBoolValue>`), so raw-poking does nothing — a node only applies its effect when *executed*. Canonical actuation: configure inputs via `SetPropertyValue`, then `SignalInputPin(node, CRC32(pin), data)` (or `Activate()`). Both need on-thread, in-process execution ⇒ **Tier-2 DLL**. First node to RE end-to-end: `ZCLSetHumanoidImmuneToDamage`.
- **No cheap external instance discovery (RESOLVED).** Entity instances don't back-reference the reflection IType (§3.4); the ammo oracle is a pooled value-box, not a field. A live entity must come from a stable anchor (player/scene-context global) or **in-process enumeration (Tier-2)**. This is the gate on Tier-1 data features and is the main argument for going Tier-2.
- **Where the Glacier resolver lives — DECIDED.** Glacier methods live on `GlacierReflection` (takes `&dyn Ctx`), not on `Ctx` — keeps the shared trait UE5-shaped. Implemented this way in `glacier-host`.
- **Backend dispatch is invasive.** `Attached.session: Arc<Ue5Session>` is threaded through ~15 call sites + `main_module_base()` + Lua. Adding a 2nd backend (enum/trait) touches attach.rs/commands.rs/freeze.rs and risks the working UE5 path; it's unverifiable without a live app run. Don't build it until the Tier-1-vs-Tier-2 decision is made (a Tier-2 DLL session would look like `Ue5Session`, reusing much of the existing path).
