//! Thin wrappers around `CreateFileW`/`ReadFile`/`WriteFile`/`WaitNamedPipeW`.
//!
//! Centralised so [`crate::client`] doesn't mention any Win32 type except
//! `HANDLE`. Each call returns [`HostError::Win32`] with the call name baked in.
//! Protocol-agnostic — identical in shape to the UE5 host's pipe layer.
...
impl PipeHandle {
    ...
    pub fn open(name: &str, overall_timeout: Duration) -> Result<Self> {
        ...
        loop {
            // SAFETY: `wide` is null-terminated; the pointer is stable for the
            ...
            match result {
                ...
                Ok(_) | Err(_) => {
                    ...
                    if code == ERROR_PIPE_BUSY {
                        // SAFETY: `wide` is null-terminated and lives across the call.
    ...
    pub fn write_all(&self, buf: &[u8]) -> Result<()> {
        ...
        while !remaining.is_empty() {
            ...
            // SAFETY: handle valid; lpBuffer is a slice; out ptr is valid.
    ...
    /// Read exactly `buf.len()` bytes. Returns `Disconnected` on EOF.
    pub fn read_exact(&self, buf: &mut [u8]) -> Result<()> {
        let mut filled: usize = 0;
        while filled < buf.len() {
            let mut got: u32 = 0;
            // SAFETY: handle valid; tail slice non-empty; out ptr valid.
...
// threads. The `GlacierClient` keeps its `PipeHandle` behind `&mut self`, so
// no aliasing exists.
