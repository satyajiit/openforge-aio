...
//! [`openforge_glacier_protocol::Request::DrainLog`].
//!
...
use openforge_glacier_protocol::LogLevel;
...
impl LogRing {
    ...
    /// Returns true if `level` is enabled (i.e. `level <= configured` and the
    /// configured level isn't `Off`).
    #[inline]
