...
1. Grab the latest **`openforge-vX.Y.Z-windows-x64.zip`** from **[Releases](https://github.com/satyajiit/openforge-aio/releases)**.
2. Extract both files (`openforge.exe` + `batman_lod_dll.dll`) into the same folder. They must live side-by-side — the trainer looks for the DLL next to its own .exe.
3. Right-click `openforge.exe` → **Run as administrator** (game memory access needs elevation; OpenForge prompts if you forget).
4. Launch your game.
5. **Load into the game proper** — start a new game, load a save, or get to the point where you're actually controlling your character. **Don't Attach on the main menu or during a loading screen.**
6. Hit **Attach** in OpenForge. Toggle cheats. Be merciful with the cutscenes.
